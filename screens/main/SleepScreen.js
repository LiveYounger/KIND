...
import {
  ...
  setActiveSleep,
  getSleepTabs
} from '../../store/actions/SleepActions';
import { sleepsSelector } from '../../store/selectors/SleepSelector';
...
const SleepScreen = ({ navigation }) => {
  ...
  useEffect(
    () => {
      if (selectedCategory !== null) {
        dispatch(getSleepTabs(selectedCategory));
        dispatch(
          getSleeps(
            selectedTab < 0
  ...
  useEffect(
    () => {
      if (allCategories) {
        const screenCategory = allCategories.find(category => category.name === 'Sleep');
        if (screenCategory) {
  ...
  const sleeps = useSelector(sleepsSelector());
  const user = useSelector(userSelector());

  useEffect(
    () => {
      if (sleeps.isAll) {
        const tabs = generateCategoryTabs(sleeps.sleepTabs);
  ...
  const handleSleepPress = sleep => {
    dispatch(setActiveSleep(sleep));
    navigation.navigate(sleep.is_premium && !user.is_premium ? 'UnlockPremium' : 'ActiveSleep', {
      sleepId: sleep.id
    });
  };
  ...
  const fetchMore = () => {
    if (sleeps.next && !loader) dispatch(loadMoreSleeps(sleeps.next));
  };
    ...
    <View style={[screenWrap, screenBlack]}>
      ...
      <View style={[screenContent, screenPadTop]}>
        <View style={[uFlexRow, uFlexAlignCenter, uFlexJustifyBetween, uGapBottom]}>
          <Text style={[screenTitle, screenTitleSm]}>{$t('main.sleep')}</Text>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Info', { infoId: 'info-sleeps' })}
          >
        ...
        <InfiniteScrollList
          ...
          data={loader ? [] : sleeps.results}
          refresh={refresh}
          refreshing={false}
          fetchMore={fetchMore}
          onItemPress={handleSleepPress}
          navigation={navigation}
          premiumButtonVisible={!user.is_premium && !!sleeps.results.length}
